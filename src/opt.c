...
void lbfgs_save(int n, float *x, float *g, float **sk, float **yk, opt_t *opt)
{
  ...
  if(opt->kpair < opt->npair){
    ...
    opt->kpair++; // always we have: kpair<=npair
  }else{
...
void lbfgs_descent(int n, float *g, float *d, float **sk, float **yk, float *q, float *rho, float *alpha, opt_t *opt)
{
  int i, j;
  float tmp0, tmp1, gamma, beta;

  //safeguard a descent direction from negative gradient, d=-g
  tmp0=opt->kpair>0?l2norm(n, opt->sk[opt->kpair-1]):0;
  tmp1=opt->kpair>0?l2norm(n, opt->yk[opt->kpair-1]):0;
  if(!( tmp0>0. && tmp1>0.)){
...
void lbfgs_descent1(int n, float *g, float *q, float *rho, float *alpha, float **sk, float **yk, opt_t *opt)
{
  int i, j;
  float tmp0, tmp1;
  ...
  //safeguard a descent direction from negative gradient, d=-q=-g
  opt->loop1 = false;
  tmp0 = l2norm(n, opt->sk[opt->kpair-1]);
  tmp1 = l2norm(n, opt->yk[opt->kpair-1]);
  if(!( tmp0>0. && tmp1>0.)) return;//no need for 2nd loop

  for(i=opt->kpair-1; i>=0; i--){
    tmp0 = dotprod(n, yk[i], sk[i]);
    tmp1 = dotprod(n, sk[i], q);
    rho[i] = 1./tmp0;//calculate rho
    alpha[i] = rho[i]*tmp1;
    for(j=0; j<n; j++) q[j] -= alpha[i]*yk[i][j];
  }
  opt->loop1 = true;//done the 1st loop
}

/*< calculate search direction (2nd loop of the two-loop recursion) >*/
void lbfgs_descent2(int n, float *g, float *q, float *rho, float *alpha, float **sk, float **yk, opt_t *opt)
{
  int i, j;
  float tmp0, tmp1, gamma, beta;
  ...
  flipsign(n, opt->q, opt->d); //descent direction d=-q where q=H^{-1}g
}
		 ...
		 opt_t *opt) //pointer of l-BFGS optimization parameters
{
  ...
  for(opt->ils=0; opt->ils<opt->nls; opt->ils++){
    ...
    if(fcost > fxx){
      ...
      else                opt->alpha *= 2.0; //extend the search interval if alpha2==0
...
/*< solve Newton equation by conjugate gradient iterations >*/
	      ...
	      opt_t *opt) //pointer of l-BFGS optimization parameters
{
  ...
  rs0 = rsold;
  for(k=0; k<opt->ncg; k++){
    if(opt->verb) printf("===== CG %d, |r|^2=%e, fcost=%e =====\n", k, rsold, fcost);
