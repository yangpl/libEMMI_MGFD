...
Short answer: pick the *physical conservation statement* you care about (flux continuity, conserved current, total resistance, energy, etc.), write it for the heterogeneous medium and for a *homogeneous* effective medium, set them equal, and solve for the effective property. The resulting average depends on the *physics* (series vs parallel, normal vs tangential to interfaces) and the geometry.

Below I walk through a concrete, commonly-used example (1D layered conduction) and a finite-volume face average useful for discretizations.
...
**Physics / conservation used:** the same vertical flux $J$ (current density) flows through both layers (steady state, series). Ohm/Fourier law locally: $J = \sigma  \frac{\Delta V_{layer}}{t_{layer}}$. The total voltage drop equals sum of drops:
...
* $T = 2+3 = 5$.
* $t_1/\sigma_1 = 2 / 0.10 = 20$.
* $t_2/\sigma_2 = 3 / 0.01 = 300$.
* Sum = $20 + 300 = 320$.
* $\sigma_{\rm eff} = 5 / 320 = 0.015625$ S/m.
...
**Interpretation:** harmonic mean arises because you conserve flux (same $J$) and add the drops.
...
(for two equal-width paralell pieces; in general a weighted arithmetic mean). So **parallel → arithmetic mean**, **series → harmonic mean**.
...
* numerator $=1+1=2$.
* denominator $=1/0.10 + 1/0.01 = 10 + 100 = 110$.
* $\sigma_f = 2/110 \approx 0.0181818$ S/m.

This is exactly what many FD/FV codes use for face transmissibility when coefficients are discontinuous.


\subsection{ How to do this in general (recipe)}

1. **Identify the governing (conservation) equation** for your problem (e.g. $\nabla\!\cdot(\sigma\nabla\phi)=0$, conservation of mass, energy, momentum, etc.).
2. **Integrate or write the balance** over the region of interest (control volume, series elements, network).
3. **Assume piecewise-constant properties** on subregions (or prescribe spatial variation) and express the global response (total flux, total resistance, total energy) in terms of subregion properties.
4. **Define the effective property** by requiring the homogeneous medium with that property gives the same global response.
5. **Solve algebraically** to get the averaging formula. Inspect the result: harmonic, arithmetic, geometric, or a weighted version will appear depending on which quantity was conserved.
...
* Conservation of *flux* through series layers → **harmonic mean** (weighted by thickness).
* Conservation of *potential* across parallel paths → **arithmetic mean** (weighted by area/width).
* Finite-volume face average enforcing flux continuity → **distance-weighted harmonic mean**.
* Procedure: write conservation → express global response → define effective property → solve.
