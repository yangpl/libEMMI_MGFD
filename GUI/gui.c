...
static gboolean load_inputpar(App *app, gboolean quiet) {
  ...
  set_text_to_view(app->text_inputpar, content);
  if (!quiet) {
    set_status(app, "Loaded inputpar.txt");
    append_log(app, "[GUI] Loaded inputpar.txt.\n");
  }
  g_free(content);
  g_free(path);
  return TRUE;
...
static gboolean save_inputpar(App *app, GError **err) {
  gchar *path = build_inputpar_path(app);
  gchar *content = get_text_from_view(app->text_inputpar);
...
static gboolean run_plot_hook(App *app) {
  ...
  if (!g_spawn_sync(run_dir, argv, NULL, 0, NULL, NULL, &out, &err, &exit_status, &gerr)) {
    gchar *msg = g_strdup_printf("[GUI] Plot hook failed to start: %s", gerr ? gerr->message : "unknown");
    append_log(app, msg);
    append_log(app, "\n");
    g_free(msg);
...
static void on_child_exit(GPid pid, gint status, gpointer data) {
  ...
  if (WIFEXITED(status) && WEXITSTATUS(status) == 0) load_inputpar(app, TRUE);
  set_status(app, "Idle");
}

static void on_load_inputpar(GtkButton *btn, gpointer data) {
  App *app = (App *)data;
  load_inputpar(app, FALSE);
  (void)btn;
}

static void on_save_inputpar(GtkButton *btn, gpointer data) {
  App *app = (App *)data;
  GError *err = NULL;
  if (save_inputpar(app, &err)) {
    set_status(app, "Saved inputpar.txt");
    append_log(app, "[GUI] Saved inputpar.txt.\n");
  } else {
...
static void on_start_run(GtkButton *btn, gpointer data) {
  ...
  if (!g_file_test(bin, G_FILE_TEST_IS_REGULAR)) {
    set_status(app, "Binary path is invalid.");
    append_log(app, "[GUI] Invalid binary path.\n");
    return;
  }
  if (!save_inputpar(app, &err)) {
  ...
  gchar *inputpar_path = build_inputpar_path(app);
  gchar *qbin = g_shell_quote(bin);
  gchar *qinput = g_shell_quote(inputpar_path);
  gchar *run_cmd = g_strdup_printf("mpirun -n %d %s $(cat %s)",
                                   gtk_spin_button_get_value_as_int(GTK_SPIN_BUTTON(app->spin_nproc)),
                                   qbin, qinput);
  gchar *spawn_argv[] = {(gchar *)"/bin/bash", (gchar *)"-lc", run_cmd, NULL};
  gint out_fd = -1, err_fd = -1;

  append_log(app, "[GUI] Running: ");
  append_log(app, run_cmd);
  append_log(app, "\n");

  if (!g_spawn_async_with_pipes(run_dir, spawn_argv, NULL, G_SPAWN_DO_NOT_REAP_CHILD, NULL, NULL, &app->child_pid,
...
static void populate_default_paths(App *app) {
  ...
  gchar *run_dir = g_build_filename(project, "run_modelling", NULL);
  gchar *bin = g_build_filename(project, "bin", "main", NULL);
...
int main(int argc, char **argv) {
  ...
  gtk_window_set_title(GTK_WINDOW(app.window), "libEMMI_MGFD GTK Launcher");
  ...
  make_labeled_entry(grid, "Binary", 2, &app.entry_bin);
  ...
  gtk_spin_button_set_value(GTK_SPIN_BUTTON(app.spin_nproc), 1);
  ...
  GtkWidget *btn_load_input = gtk_button_new_with_label("Load inputpar.txt");
  GtkWidget *btn_save_input = gtk_button_new_with_label("Save inputpar.txt");
  app.btn_run = gtk_button_new_with_label("Run");
  app.btn_stop = gtk_button_new_with_label("Stop");
  gtk_widget_set_sensitive(app.btn_stop, FALSE);
  gtk_box_pack_start(GTK_BOX(control_row), btn_load_input, FALSE, FALSE, 0);
  gtk_box_pack_start(GTK_BOX(control_row), btn_save_input, FALSE, FALSE, 0);
  gtk_box_pack_start(GTK_BOX(control_row), app.btn_run, FALSE, FALSE, 0);
  gtk_box_pack_start(GTK_BOX(control_row), app.btn_stop, FALSE, FALSE, 0);
  ...
  gtk_widget_set_hexpand(sw_input, TRUE);
  gtk_widget_set_vexpand(sw_input, TRUE);
  app.text_inputpar = gtk_text_view_new();
  gtk_text_view_set_monospace(GTK_TEXT_VIEW(app.text_inputpar), TRUE);
  gtk_container_add(GTK_CONTAINER(sw_input), app.text_inputpar);
  gtk_container_add(GTK_CONTAINER(frame_input), sw_input);
  gtk_paned_add1(GTK_PANED(main_paned), frame_input);
  ...
  gtk_widget_set_hexpand(sw_log, TRUE);
  ...
  gtk_text_view_set_monospace(GTK_TEXT_VIEW(app.text_log), TRUE);
  ...
  gtk_entry_set_text(GTK_ENTRY(app.entry_plot_cmd), "python3 4_plot_emdata.py");
  ...
  gtk_paned_set_position(GTK_PANED(main_paned), 520);
  ...
  g_signal_connect(btn_load_input, "clicked", G_CALLBACK(on_load_inputpar), &app);
  g_signal_connect(btn_save_input, "clicked", G_CALLBACK(on_save_inputpar), &app);
